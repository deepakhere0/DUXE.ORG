...
import {
  collection, doc, getDoc, getDocs, setDoc, addDoc, updateDoc, deleteDoc, query, where, orderBy, limit, serverTimestamp, increment
...
export const listDocs = async (collectionName, { filters = [], sort = null, take = 20 } = {}) => {
  ...
  filters.forEach((f) => { if (f) q = query(q, where(f.field, f.op || '==', f.value)); });
...
export const Users = {
  collection: 'users',
  async upsert(uid, data) { return createDoc(this.collection, { uid, ...data }, uid); },
  async get(uid) { return readDoc(this.collection, uid); },
  async update(uid, data) { return updateDocById(this.collection, uid, data); },
};

export const Universities = {
  collection: 'universities',
  async list() { return listDocs(this.collection, { sort: { field: 'name', dir: 'asc' }, take: 500 }); },
};

export const Departments = {
  collection: 'departments',
  async listByUni(uniId) { return listDocs(this.collection, { filters: [{ field: 'uniId', value: uniId }], sort: { field: 'name', dir: 'asc' }, take: 1000 }); },
};

export const Notes = {
  collection: 'notes',
  async listApproved({ queryText = '', universityId = '', departmentId = '', subject = '', semester = '' } = {}) {
...
export const AIJobs = {
  collection: 'aiJobs',
  async create(job) { return createDoc(this.collection, { status: 'queued', ...job }); },
  async update(id, data) { return updateDocById(this.collection, id, data); },
  async get(id) { return readDoc(this.collection, id); },
};

export const Internships = {
  collection: 'internships',
  async list() { return listDocs(this.collection, { sort: { field: 'postedAt', dir: 'desc' }, take: 100 }); },
};
...
export const Analytics = {
  ...
  async bumpCounter(key) { return createDoc(this.collection, { key }, key).catch(() => {});
