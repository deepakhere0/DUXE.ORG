...
const NoteCard = ({
  ...
  return (
    <>
      <div
        className={`bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden border border-gray-100 relative ${className}`}
      >
        ...
        <div className="p-6">
          ...
          <div className="flex items-start justify-between mb-3 pr-16">
            <div className="flex-1 min-w-0">
              ...
              {note.courseCode && (
                <span
                  className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getSubjectColor(note.courseCode)}`}
                >
                  {note.courseCode}
                </span>
              )}
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center space-x-2">
              <div className="flex items-center space-x-1">{getRatingStars(note.ratingAvg)}</div>
              <span className="text-sm text-gray-600">
                {note.ratingAvg ? note.ratingAvg.toFixed(1) : '0.0'}
                {note.ratingCount && <span className="text-gray-500"> ({note.ratingCount})</span>}
              </span>
            </div>
