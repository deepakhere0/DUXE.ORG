...
param(
    ...
    [switch]$SkipTests,
    [switch]$SkipBuild,
    [string]$Platform = "vercel" # vercel or netlify
)
...
if (-not (Test-Path ".env.local")) {
    Write-Host "⚠️  .env.local file not found. Make sure to set up environment variables." -ForegroundColor Yellow
}
...
if (-not $SkipTests) {
    Write-Host "`n🧹 Running linter..." -ForegroundColor Green
    npm run lint
    if ($LASTEXITCODE -ne 0) {
...
if (-not $SkipBuild) {
    Write-Host "`n🏗️  Building project..." -ForegroundColor Green
    npm run build
    if ($LASTEXITCODE -ne 0) {
...
if (Test-Command "firebase") {
    ...
    Write-Host "`n🔥 Firebase deployment options:" -ForegroundColor Cyan
    Write-Host "  firebase deploy --only firestore:rules    # Deploy Firestore rules"
    Write-Host "  firebase deploy --only storage           # Deploy Storage rules"
    Write-Host "  firebase deploy --only functions         # Deploy Cloud Functions"
    Write-Host "  firebase deploy                          # Deploy all Firebase services"
} else {
    Write-Host "⚠️  Firebase CLI not found. Install with: npm install -g firebase-tools" -ForegroundColor Yellow
}

# Platform-specific deployment
Write-Host "`n🌐 Deployment platform: $Platform" -ForegroundColor Cyan
